...
- 建立 59 个时期化官名概念、99 条主张和 115 条证据关联；reviewed 主张证据覆盖率为 100%。
...
- 增加 7 条单元／集成测试和 6 条 Chromium 关键用户旅程；桌面和 390px 移动端完成首轮视觉复核。
- 将 Pages workflow 改为先执行格式、lint、严格类型、数据校验、单测、构建和 E2E，再发布经过验证的 `dist/`。

## 当前工作

- 补齐多方案品秩、方案内层级和带置信度／争议状态的跨方案映射。
...
Validated 0.1.0-research.1: 59 title concepts, 99 assertions, 115 evidence links.
...
1. 补品秩方案并将已证官名连接到机构／品秩。
2. 继续整理苏轼年谱材料可得性和明确的 coverage 缺口。
3. 完成比较器、路径模拟和数据下载的研究级交互。
4. 建立性能预算并进行 WCAG 审计。
